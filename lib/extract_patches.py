...
def get_data_training(DRIVE_train_imgs_original,
    ...
    train_bordermasks = load_hdf5(DRIVE_train_bordermasks) #bordermasks always the same
    ...
    if fine_tuning==False:
        ...
        patches_imgs_train, patches_masks_train = extract_random_ft(train_imgs,train_masks,train_bordermasks,patch_height,patch_width,N_subimgs,inside_FOV)
        data_consistency_check(patches_imgs_train, patches_masks_train)
...
def get_data_testing(DRIVE_test_imgs_original, DRIVE_test_groudTruth, Imgs_to_test, patch_height, patch_width):
    ...
    #paint border is to extend the image to make the dimension compatible with the patch dimension
...
def extract_random_ft(full_imgs,full_masks,bordermasks, patch_h,patch_w, N_patches, inside=True):
    if (N_patches%full_imgs.shape[0] != 0):
        print ("N_patches is ")
        print (N_patches)
        print ("full_imgs.shape[0]")
    ...
    for i in range(full_imgs.shape[0]):  #loop over the full images
        ...
        while k <patch_per_img:
            ...
            #check whether the patch is fully contained in the FOV
            if inside==True :
                if inside_FOV_DRIVE(i, x_center, y_center, bordermasks)==False:
                    continue
            patch = full_imgs[i,:,y_center-int(patch_h/2):y_center+int(patch_h/2),x_center-int(patch_w/2):x_center+int(patch_w/2)]
            patch_mask = full_masks[i,:,y_center-int(patch_h/2):y_center+int(patch_h/2),x_center-int(patch_w/2):x_center+int(patch_w/2)]
            patches[iter_tot]=patch
...
def recompone(data,img_h,img_w):
